...
export const sortCategories = async (
  ...
  try {
    ...
    const updateCategoriesQuery = `UPDATE categories
            SET
            sortOrder=(CASE uuid 
                ${categoryList.map((c: Category, i: number) => 
                `WHEN ${SqlString.escape(c.uuid)} THEN ${SqlString.escape(i)} `).join(" ")}
            END)
            WHERE uuid IN (${categoryList.map((c: Category) => SqlString.escape(c.uuid)).join(",")})`
    try {

...
import {TODO_PRIORITY, TODO_SIZE, TODO_STATUS} from '@/app/components/TodoItem/types'
...
type Todo_Progress_Params = {
    name: string,
    category: Category,
    user_id: string
}
export const addProgress = async (req: NextApiRequest, res: NextApiResponse) => {
    try {
        const { name, category, user_id } =
            req.body
        ...
        const insertTodoQuery = `INSERT into todos
            VALUES (
                ...
                ${SqlString.escape(dateIsoToSql(dayjs(new Date()).startOf('day').toISOString()))},
